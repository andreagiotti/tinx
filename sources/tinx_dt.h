...
/*
#if defined NDEBUG
#define valid(S) ((S).e.lc >= 0 || (S).e.ls)
#else
...
#define wait_deadline(KB) pthread_cond_wait(&(KB)->cond_done, &(KB)->mutex_barrier)
#define signal_deadline(KB) pthread_cond_signal(&(KB)->cond_done)
...
typedef struct k_base
{
  ...
  d_time curr_time;	/* Do not reorder */
  d_time all_taken;
  d_time abandon;
  stream *skip_ios;
