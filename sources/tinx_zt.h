...
#define position_of(KB, S) (&(link_of((S).e).shared[index_of(KB, S)]))
#define position2_of(KB, S) (&(link_of((S).e).shared2[index_of(KB, S)]))
...
#define safe_state(KB, S) { if(!shared(S)) { if(!is_stated(KB, S)) state(KB, S); } else update_stream_cache(KB, S, TRUE); }
...
stream *open_stream(char *name, stream_class sclass, arc e, d_time bs, d_time offset, io_type stype, bool sys5,
